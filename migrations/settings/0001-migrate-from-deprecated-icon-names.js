...
function convertIconName(iconName) {
  ...
  // Replace FA4 prefixes
  Object.entries(fa_4_icon_map).forEach(([key, value]) => {
    newName = newName.replace(key, value);
  });

  // Handle FA5 remapping
  let lookupName = newName;
  let prefix = null;

  if (lookupName.startsWith("far-")) {
    prefix = "far";
    lookupName = lookupName.substring(4);
  } else if (lookupName.startsWith("fab-")) {
    prefix = "fab";
    lookupName = lookupName.substring(4);
  }

  if (fa5_remaps.hasOwnProperty(lookupName)) {
    let remappedName = fa5_remaps[lookupName];
    return prefix ? `${prefix}-${remappedName}` : remappedName;
  }
